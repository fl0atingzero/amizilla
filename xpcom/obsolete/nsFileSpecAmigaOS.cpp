...
//----------------------------------------------------------------------------------------
{
    struct PRFileInfo st;
    return !mPath.IsEmpty() && PR_GetFileInfo(mPath, &st) == PR_SUCCESS;
} // nsFileSpec::Exists
...
{
	struct PRFileInfo st;
  if (!mPath.IsEmpty() && PR_GetFileInfo(mPath, &st) == PR_SUCCESS)
    outStamp = st.modifyTime;
  else
    outStamp = 0;
...
{
    struct PRFileInfo st;
    return !mPath.IsEmpty() && PR_GetFileInfo(mPath, &st) == PR_SUCCESS && st.type == PR_FILE_FILE;
} // nsFileSpec::IsFile
...
{
    struct PRFileInfo st;
    return !mPath.IsEmpty() && PR_GetFileInfo(mPath, &st) == PR_SUCCESS && st.type == PR_FILE_DIRECTORY;
...

{
    ...
    mDir = PR_OpenDir((const char*)nsFilePath(inDirectory));
    ++(*this);
